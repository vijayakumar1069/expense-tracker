            ...
            <div className="grid grid-cols-7 gap-1 h-16">
              {weeklyData.map((day, index) => (
                <div key={index} className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-[#64748b] mt-1">{day.name}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <div className="flex items-center p-2 rounded-md bg-[#ecfdf5]">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#0f172a]">{weeklyData.reduce((sum, day) => sum + day.income, 0)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center p-2 rounded-md bg-[#fff1f2]">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#0f172a]">{weeklyData.reduce((sum, day) => sum + day.expense, 0)}</p>
              </div>
            </div>
          </div>
        </div>
